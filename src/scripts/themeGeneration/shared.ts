...
export function resolveZedIconPath(iconName: string): string {
  return `./icons/${iconName}.svg`
}
...
export function buildFileAssociations(iconDefinitions: IconDefinition[]): {
...
export function buildFolderAssociations(iconDefinitions: IconDefinition[]): {
  ...
  return { folderNames, folderNamesExpanded }
}
...
export function applyChristmasTheme(schema: ThemeSchema, xmasIconNames: string[]): ThemeSchema {
  const theme = structuredClone(schema)
  const xmasSet = new Set(xmasIconNames)

  const makeXmasIcon = (iconName: string): string => {
    if (iconName.endsWith("_xmas")) {
      return iconName
    }
    if (!xmasSet.has(iconName)) {
      return iconName
    }
    const xmasKey = iconName + "_xmas"
    if (xmasKey in theme.iconDefinitions) {
      return xmasKey
    }
    const original = theme.iconDefinitions[iconName]
    theme.iconDefinitions[xmasKey] = {
      iconPath: original.iconPath.replace(".svg", "_xmas.svg"),
    }
    return xmasKey
  }

  theme.folder = makeXmasIcon(theme.folder)
  theme.folderExpanded = makeXmasIcon(theme.folderExpanded)

  for (const [folderPath, iconName] of Object.entries(theme.folderNames)) {
    theme.folderNames[folderPath] = makeXmasIcon(iconName)
  }
  for (const [folderPath, iconName] of Object.entries(theme.folderNamesExpanded)) {
    theme.folderNamesExpanded[folderPath] = makeXmasIcon(iconName)
  }
